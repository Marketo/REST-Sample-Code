...
public class EmailByName {
	...
	public static void main(String[] args){
		EmailByName template = new EmailByName();
		template.name = "Blank";
	...
	private String getData() {
        String data = null;
        try {
        	StringBuilder endpoint = new StringBuilder(marketoInstance + "/rest/asset/v1/email/byName.json?access_token=" + getToken() + "&name=" + name);
        	if(status != null){
        		endpoint.append("&status=" + status);
        	}
        	if (folder != null){
        		endpoint.append("&folder=" + folder);
        	}
