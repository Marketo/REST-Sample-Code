...
public class GetSegmentations {
	...
	public int maxReturn;//max number of results to return, default 20, max 200
	public String status;//optional status filter, draft or approved
	
	public static void main(String[] args){
		GetSegmentations segmentations = new GetSegmentations();
		segmentations.maxReturn = 15;
		String result = segmentations.getData();
		System.out.println(result);
	}
	private String getData() {
        String data = null;
        try {
        	StringBuilder endpoint = new StringBuilder(marketoInstance + "/rest/asset/v1/segmentation.json" + "?access_token=" + getToken());
        	if (offset > 0){
        		endpoint.append("&offset=" + String.valueOf(offset));
        	}
        	if (maxReturn > 0){
        		endpoint.append("&maxReturn=" + String.valueOf(maxReturn));
        	}
        	if (status != null){
