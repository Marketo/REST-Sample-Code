...
public class BrowseFolders {
	...
	public JsonObject root; //JSON representation with two members, ID and Type, required
	public String offset; //offset returned from previous call
	public Integer maxDepth; //maximum tree depth to browse
	public Integer maxReturn; //maximum number of records to return, max 100, default 20
	public String workSpace; //name of workspace, required if subscription has workspaces
	
	
	public static void main(String[] args){
		BrowseFolders folders = new BrowseFolders();
		folders.root = new JsonObject()
							.add("id", 1071)
							.add("type", "Program");
		String result = folders.getData();
	...
	private String getData() {
        ...
        try {
        	//Assemble the URL
        	StringBuilder endpoint = new StringBuilder(marketoInstance + "/rest/asset/v1/folders.json?access_token=" + getToken() + "&root=" + root);
        	//add optional params
        	if(offset != null){
        		endpoint.append("&offset=" + offset);
        	}
        	if(maxDepth != null){
        		endpoint.append("&maxDepth=" + maxDepth);
        	}
        	if(maxReturn != null){
        		endpoint.append("&maxReturn=" + maxReturn);
        	}
        	if(workSpace != null){
        		endpoint.append("&workSpace=" + workSpace);
        	}
