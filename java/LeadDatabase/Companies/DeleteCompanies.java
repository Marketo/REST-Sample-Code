...
public class DeleteCompanies {
	...
	//It will REALLY delete companies permanently
	...
	public static void main(String[] args){
		DeleteCompanies deleteCompanies = new DeleteCompanies();
		deleteCompanies.externalIds = new String[]{"CompanyTest1"};
		String result = deleteCompanies.postData();
	...
	public String postData(){
		...
		try {
			//Assemble the URL
			String endpoint = marketoInstance + "/rest/v1/companies/delete.json?access_token=" + getToken();
	...
	public JsonObject buildRequest(){
		...
		if (this.dedupeBy == "idField"){
			...
			for(i = 0; i < externalIds.length; i++){
				JsonObject jo = new JsonObject()
									.add("externalcompanyid", externalIds[i]);
				inputArray.add(jo);
			}
		}
